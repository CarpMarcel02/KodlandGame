...
class Player:
    ...
    def __init__(self, spawn_x, spawn_y, size=28, speed=160.0):
        self.rect = Rect(
            spawn_x + (tiles.TILE - size)//2,
            spawn_y + (tiles.TILE - size)//2,
            size, size
        )
        self.speed = speed
        ...
        self.anim_walk = {
            "UP":    _load_dir("player/hero_walk_down",  FRAMES),  
            "LEFT":  _load_dir("player/hero_walk_left",  FRAMES),
            "DOWN":  _load_dir("player/hero_walk_right", FRAMES),  
            "RIGHT": _load_dir("player/hero_walk_up",    FRAMES),  
        }
        self.facing = "DOWN"
        self.anim_ix = 0
        self.anim_t = 0.0
    ...
    def draw(self, ctx, cam_x, cam_y):
        ...
        # debug hitbox :
        # ctx.screen.draw.filled_rect(self.rect.move(-cam_x, -cam_y), (230,230,240))
